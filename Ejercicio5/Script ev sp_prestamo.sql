...
begin
	...
	begin
		...
		insert into ev_amortizacion (
			codigo, 		codigo_prestamo, 	num_cuota, 		cuota, 			capital_inicio, 	interes, 			capital, 			saldo, 		pago, 	estado, 	cuota_adeuda)
		values(
			@w_cod_cuota,	@w_codigo_prest,	@w_contador,	@w_valor_cuota,	@W_capital_ini,		@w_interes_cuota, 	@w_abono_capital,	@w_saldo,	0,		'P',		@w_valor_cuota)
...
begin
	...
	--Primer select prestamo y cliente
	select
		ec.cedula,
		ec.nombre + ' ' + ec.apellido as "Nombre cliente",
		ep.codigo,
		round(ep.monto,2),
		ep.plazo,
		round(ep.interes,2)
	from ev_prestamo ep, ev_clientes ec 
	where codigo = @i_codigo and ec.cedula =ep.cedula 
	
	--Segundo select con tabla amortizacion
	select
		codigo,
		codigo_prestamo,
		num_cuota,
		round(cuota,-4),
		round(capital_inicio,-4),
		round(interes,2),
		round(capital,2),
		round(saldo,2),
		round(pago,2),
		estado,
		round(cuota_adeuda,2)
	from ev_amortizacion ea 
	where ea.codigo_prestamo  = @i_codigo
end

--Q=Recuerar todos los prestamos de un cliente
if @i_operacion = 'Q'
begin
	if @i_cedula is null
	begin
	...
	select
		...
		round(ep.interes,2)
	from ev_prestamo ep, ev_clientes ec 
	where ec.cedula = @i_cedula and ec.cedula = ep.cedula 
